...
La PC cumple con los requisitos básicos para ejecutar IDEs, entornos locales y herramientas de diseño. El respaldo está resuelto a través de backups en la nube.
...
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| Rol de usuario | Alumno                                                                                                                                 | Docentes, administradores y alumnos                                                                  |
| Funcionalidad  | Resolución de misiones offline con entorno gamificado, detección de dificultades y mejora para las mismas mediante misiones especiales | Gestión de instituciones, cursos, docentes. Progreso y estadísticas, Reportes y Sesiones de refuerzo |
...
El videojuego contará con una base de datos local en SQLite para almacenar el progreso de cada alumno de forma offline. Luego, al finalizar una misión o cerrar sesión, sincronizará los datos de progreso (misiones completadas, intentos, estrellas y experiencia) y registro de errores con una base de datos global en PostgreSQL a través de una API proporcionada por el backend NestJS. Cada alumno tendrá su propia base local; la base central del sistema agregará todos los datos recibidos desde los distintos juegos.
...
- **Docker**: se instalará la ultima versión estable de Docker Desktop 4.43.2 en Windows para las pruebas de despliegue.
...
El desarrollo inicial se realizará localmente utilizando PostgreSQL como sistema de gestión de base de datos para el backend web, mientras que SQLite se reservará exclusivamente para la base de datos local del videojuego. Desde etapas tempranas se incorporará la utilización de Docker para contenerizar la aplicación, lo que permitirá mantener un entorno de desarrollo consistente y facilitará la configuración, prueba y despliegue progresivo del sistema. De esta forma, se optimiza el proceso de instalación, actualización y mantenimiento de cada instancia en las distintas instituciones, garantizando portabilidad y escalabilidad sin sacrificar la agilidad del desarrollo. La adopción temprana de Docker facilitará la transición hacia entornos productivos más complejos y automatizados.
...
Se identificaron los principales desafíos técnicos: curva de aprendizaje de Godot y NestJS, integración segura con JWT y sincronización progresiva. Sin embargo, con el cronograma planteado, el uso de recursos formativos gratuitos, y una planificación basada en pruebas incrementales, los riesgos son gestionables.

Además, se incorpora desde las primeras etapas el uso de Docker como herramienta de contenerización, tanto para el desarrollo local como para los futuros despliegues en instituciones educativas. Esto permite asegurar la consistencia de los entornos, facilita la instalación y actualización del sistema, y habilita una transición fluida hacia escenarios de producción. La infraestructura disponible, las herramientas seleccionadas y la arquitectura técnica adoptada permiten avanzar con seguridad en el desarrollo del sistema.
...
**Despliegue inicial**
No se contratará ningún servidor en la nube. El sistema se desplegará en entorno local utilizando Docker, lo que permite contenerizar y ejecutar tanto el backend como el frontend y la base de datos PostgreSQL de forma integrada para pruebas y demostración. Esta infraestructura es suficiente para validar el funcionamiento completo del sistema sin incurrir en costos adicionales.

**Despliegue institucional (futuro)**
Una vez finalizado el desarrollo, se prevé el despliegue del sistema en las instituciones educativas interesadas. En este contexto, se utilizará Docker para facilitar la instalación, actualización y mantenimiento de cada instancia local, sin necesidad de conexión a servidores remotos. Esta solución evita costos de infraestructura en la nube y permite el funcionamiento en entornos desconectados o con acceso limitado a Internet.
...
- **Autenticación con Google (OAuth 2.0)** - Opción “Registrarse con Google” - Tras aceptar permisos (profile, email), se completa perfil con datos faltantes (avatar, usuario y contraseña).
...
- **Frecuencia de interacción**
  - **Objetivo mínimo**: 1 ingreso diario para resolver misiones.
...
- Pasos para creación de curso, asi como tambien la modificación o baja.
...
- PGA no requiere conexión permanente a internet para funcionar: todas las misiones, narrativa y recursos gráficos están empaquetados localmente en el juego.
- Sin embargo, se necesita conexión para:
  - Iniciar sesión (primer uso o tras cierre de sesión).
  - Enviar datos de progreso y dificultades a la base de datos compartida.
...
### 3.9 Despliegue y Mantenimiento del Sistema
El sistema será distribuido mediante instalaciones independientes en cada institución educativa que lo adquiera, evitando así la necesidad de una única infraestructura centralizada o modelo multi-tenant. Para facilitar el despliegue, mantenimiento y futuras actualizaciones, se utilizará **Docker** como herramienta de contenedorización. Esto permitirá empaquetar toda la aplicación (backend, frontend y base de datos) en imágenes estandarizadas, garantizando un entorno de ejecución uniforme en todos los equipos.

Cada instalación podrá ejecutarse localmente con un simple archivo docker-compose.yml, simplificando el proceso técnico para las escuelas. Además, se contempla el uso de herramientas como Watchtower, que permiten la actualización automática de los contenedores cuando se publiquen nuevas versiones en el repositorio de imágenes. Esto asegurará que las mejoras, correcciones de errores y nuevas funcionalidades puedan ser aplicadas de forma ágil y sin intervención técnica compleja por parte de las instituciones.

Este enfoque asegura escalabilidad, mantenibilidad y facilidad de distribución, especialmente útil considerando que cada institución puede tener requerimientos específicos y operar de forma autónoma.

---

### 3.10 Conclusión de Factibilidad Operativa
...
- Se definirá canal de soporte en Discord (grupo cerrado) y correo de contacto
...
**5. Despliegue y actualizaciones simplificadas**
- Cada institución contará con su propia instancia de PGA, evitando la complejidad de una infraestructura centralizada.
- Al usar Docker y archivos docker-compose.yml, el sistema puede instalarse de forma local y estandarizada, incluso por personal no técnico.
- Las actualizaciones podrán distribuirse fácilmente mediante imágenes Docker publicadas, permitiendo mejoras continuas sin interrumpir el servicio.
- Se prevé el uso de herramientas como **Watchtower**, que detectan y aplican automáticamente nuevas versiones, asegurando un mantenimiento ágil y sin intervención manual en cada copia distribuido.